...
对外提供:
    submit_decision_table — 对候候选表执行「冲突检测 → 提交/中断」，供 update_decision_table
...
def _commit_table(thread_id: str, rows: list[DecisionRow]) -> str | None:
    """把候选/已决表原子写入并返回新 version（受保护 helper）。"""
    return rewrite_decision_table(str(thread_id), rows)
...
def _confirm_interrupt(candidate: list[DecisionRow], existing: list[DecisionRow], reasons: list[str]) -> dict:
    ...
    payload = {
        ...
        "candidate": [
            {"requirement": row.requirement, "decision": row.decision, "priority": row.priority}
    ...
    *,
    ...
    if verdict.action == DecisionTableAction.CONFIRM:
        ...
        if choice.get("decision") == "adopt":
            version = _commit_table(str(thread_id), candidate)
            if version is None:
