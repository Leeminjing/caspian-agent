...
工作流:
    __init__ (初始化即建目录):
    (1) 根据 user_id + thread_id 构建真实路径根目录 .caspian/users/<user_id>/threads/<thread_id>/user-data/
...
from caspian.sandbox.readers import _read_pdf, _read_docx, _read_doc
...
class LocalSandbox(Sandbox):
    ...
    def _sanitize_error(self, error: Exception, vpath: str, real_path: str) -> Exception:
        ...
        # 兜底：正则替换含 ".caspian" 的路径片段
        msg = re.sub(r'\S*\.caspian\S*', '[sandbox]', msg)
