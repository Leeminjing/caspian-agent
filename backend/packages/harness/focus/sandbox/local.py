...
工作流:
    __init__ (初始化即建目录):
    (1) 根据 user_id + thread_id 构建真实路径根目录 .focus/users/<user_id>/threads/<thread_id>/user-data/
    ...
    (2) 调用 validate_shell_command(command) 做路径安全扫描（防线 0→1→2→3→4）
    (3) 通过 shutil.which 查找目标 shell 可执行文件
    (4) 以显式调用方式（shell=False）执行命令，cwd=workspace

    PATH= 毒化检测由 SandboxAuditMiddleware 在 Agent 中间件层统一处理。
...
from focus.sandbox.path_utils import (
    REAL_ROOT,
    SKILLS_VROOT,
    SUBDIRS,
    resolve_path,
...
from focus.sandbox.readers import _read_pdf, _read_docx, _read_doc
...
class LocalSandbox(Sandbox):
    ...
    def run_shell(self, command: str, shell_type: str) -> str:
        ...
        # shell 命令路径安全检查（防线 0→1→2→3→4）
        validate_shell_command(command, shell_type)
    ...
    def _sanitize_error(self, error: Exception, vpath: str, real_path: str) -> Exception:
        ...
        # 兜底：正则替换含 ".focus" 的路径片段
        msg = re.sub(r'\S*\.focus\S*', '[sandbox]', msg)
