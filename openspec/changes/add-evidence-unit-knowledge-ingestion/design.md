...
- `knowledge/chunking.py`：只负责 Markdown/纯文本结构预切分、原子性快速闸门、token 策略和 span 集合机械验证；公开核心函数短小，解析 helpers 使用模块级受保护函数。
- `knowledge/segmentation.py`：只封装语义 span 模型调用与结构化输出解析；一个小型模型适配类实现单一 `split` 职责，原文校验仍留在 `chunking.py` 的纯函数中。
...
Markdown 扫描器是确定性的单遍状态机：维护 heading stack，并把 paragraph、list、table、fenced code block 作为结构块。候选 `content` 始终通过原文绝对 span 切取得到；解析器不对空格、标点、列表符号或代码做格式化。纯文本走段落扫描器。标题用于 `section_path`，不复制进 evidence `content`。
...
### D3: 原子性快速闸门保守放行，不能确定就切分

快速闸门仅使用可机械解释的信号：结构种类、段落/列表项数量、明显并列分隔、token 数和 hard max。满足“单结构、hard max 内、无明显多主题结构信号”的短块直接通过；其余块进入语义切分。理想区间只用于诊断和模型提示，不作为强拆/强并规则。

token 计数以一个独立 `TokenCounter` callable 注入；同一 counter 同时用于闸门和最终 hard-max 验证。具体采用当前 LangChain/模型支持的何种 tokenizer，必须在 D0 的 Context7 结果后确定，但不得改变 150–400 ideal、600 hard max、无 hard min 的规格。

Rationale：绝大多数短段落不需要额外模型调用，而不确定块不能被乐观视为原子证据。

Alternative rejected：固定每 500 tokens 切分；它会切断完整事实簇，也无法保证一个短块只含一个命题。

### D4: 模型只返回边界，代码拥有证据正文

语义切分结构化输出只包含 ordered spans 与可选 reason，不包含生成后的 chunk 文本。机械验证器执行：
...
最终 Store value 使用 `schema_version` 与 `record_type="evidence_unit"` 标识新格式。`content` 保持原文切片；`retrieval_text` 以稳定模板拼接以下非空字段：
...
→ validate all exact spans and token limits
...
新增纯函数把 `EvidenceEntry` 投影成 judge payload，只允许：`id`、`content`、`title`、`section_path`、`version`、`published_at`、`effective_at`。不得使用 `model_dump()` 直接发送整个对象，因为未来新增字段可能把 level 或 provenance 泄漏给 judge。

partial claim/span 继续以 Evidence Unit 自身 content 为局部坐标验证。govern 只消费 judge 关系与内部候选 level；score 与 metadata 均不进入等级比较。
